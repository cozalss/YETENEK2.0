...
- **LLM:** Anthropic Codex (`Codex-sonnet-4-6`, `@anthropic-ai/sdk`) — env eksikse rule-based fallback rapor; system prompt'ta prompt caching açık
...
- **API-first:** Backend sadece veri persistance + Codex proxy
...
- [x] 7 testlik batarya + sport matching + AI rapor (Codex) + rule-based fallback
...
- **Cem (AI/ML Lead):** MediaPipe entegrasyonu, test logic, sport matching, Codex API
